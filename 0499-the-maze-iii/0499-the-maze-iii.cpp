class Solution {
public:
   // A good problem try to solve it later 
string findShortestWay(vector<vector<int>>& maze, vector<int>& ball, vector<int>& hole)
{
	...
	vertices.push({0, "", ball[0], ball[1]});

	vector<vector<int>> distances(N, vector<int>(M, INT_MAX));
	distances[ball[0]][ball[1]] = 0;

	unordered_map<char, pair<int, int>> dirs{{'d', {1, 0}}, {'u', {-1, 0}}, {'r', {0, 1}}, {'l', {0, -1}}};
	...
	while(!vertices.empty())
	{
		...
		for(auto [move, dir] : dirs)
		{
			...
			if((ni != i || nj != j) && distances[i][j] + cost <= distances[ni][nj])
			{
				...
				vertices.push({distances[ni][nj], path + move, ni, nj});
			}
		}
	}
