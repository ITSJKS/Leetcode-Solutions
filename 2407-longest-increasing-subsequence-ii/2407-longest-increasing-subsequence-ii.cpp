...
#endif // SEGMENTTREE_H
//write a combine function and pass the function and an identity element into the segment tree build function

class Solution {
public:
    int id= 0;
    static int combine(int val1, int id){
            return max(val1,id);
        }
    int lengthOfLIS(vector<int>& nums, int k) {
        int N = 1e5+1;
        vector <int> v(N,0);
        SegmentTree<int> st(v,id,combine);
        int ans = 0;
        int n = nums.size();
        for(int i = 0; i <n; i++){
            int x = nums[i];
            int old = st.query(x,x);
            int q = st.query(x-k,x-1);
            if(old < q + 1){
                st.update(x,q+1);
            }
            ans = max(ans,q + 1);
...
// for each element x at index k
// I will look at my segment tree  from x - k until x-1 and find the largest value and update x to max(new,old)
